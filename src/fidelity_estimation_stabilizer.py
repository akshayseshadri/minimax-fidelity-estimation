...
class Minimax_Stabilizer_Fidelity_Estimation_Manager():
    """
    ...
    """
    def __init__(self, n, R, epsilon, epsilon_o, tol = 1e-6, print_progress = True):
        """
        ...
        # if gamma is not large enough, theoretical guarantee that AffH = sqrt(gamma) breaks down
        # the estimator is expected to work nevertheless, so just raise a warning
        if self.gamma < 0.5 * (n - 2) / (n - 1):
            warnings.warn("The number of repetitions are low enough to loose theoretical guarantees for the estimator. " +\
                          "Consider raising the number of repetitions to %d." %(np.ceil(2*np.log(2/epsilon) / np.log((n - 1)/(0.5*n - 1)))))
    ...
    def find_density_matrices_saddle_point(self):
        """
            ...
            We give explicit expression for lambda_1* and lambda_2* below.
                lambda_1* = 1/n + (1 - b*) (1 - 1/n) gamma + (1 - 1/n) sqrt(1 + b*(2 - b*) gamma/(1 - gamma))
                lambda_2* = 1/n + (1 - b*) (1 - 1/n) gamma - (1 - 1/n) sqrt(1 + b*(2 - b*) gamma/(1 - gamma))
            where gamma = (epsilon / 2)^(2 / R).

            The optimum value (b*) of the parameter b is decided as follows. We define
                b_plus  = (n/(n - 1)) [1 + sqrt((1/gamma - 1) (1 - 2/n))]
                b_minus = (n/(n - 1)) [1 - sqrt((1/gamma - 1) (1 - 2/n))]

            Observe that b_minus >= 0 if gamma >= 0.5 (d - 2)/(d - 1), while b_plus > 1 always.
            Allowed values of b lie in (0, b_minus] \cup [b_plus, 2).         
            The value of b that needs to be chosen is the allowed value that is closest to 1.
            
            b* = 1       if  b_minus >= 1
                 b_minus if |b_minus - 1| <= |b_plus - 1|
                 b_plus  if |b_minus - 1| > |b_plus - 1|

            Returns (lambda_1*, lambda_2*).
        """
        # calculating the optimum value of parameter 'b' that gives the optimum parameter for density matrices
        b_plus  = (self.n / (self.n - 1)) * (1 + np.sqrt((1/self.gamma - 1) * (1 - 2/self.n)))
        b_minus = (self.n / (self.n - 1)) * (1 - np.sqrt((1/self.gamma - 1) * (1 - 2/self.n)))

        if b_minus >= 1:
            b_opt = 1
        elif np.abs(b_minus - 1) <= np.abs(b_plus - 1):
            b_opt = b_minus
        elif np.abs(b_minus - 1) > np.abs(b_plus - 1):
            b_opt = b_plus
        ...
        self.lambda_1_opt = 1/self.n + (1 - b_opt) * (1 - 1/self.n) * self.gamma + (1 - 1/self.n) * (1 - self.gamma) * np.sqrt(1 + b_opt * (2 - b_opt) * self.gamma / (1 - self.gamma))
        self.lambda_2_opt = 1/self.n + (1 - b_opt) * (1 - 1/self.n) * self.gamma - (1 - 1/self.n) * (1 - self.gamma) * np.sqrt(1 + b_opt * (2 - b_opt) * self.gamma / (1 - self.gamma))
    ...
    def maximize_Phi_r_density_matrices(self, phi, alpha):
        """
            ...
            Tr(Omega sigma_1)       = lambda_1 + (1 - lambda_1) (n/2 - 1)/(n - 1)
            Tr(Delta_Omega sigma_1) = (1 - lambda_1) (n/2)/(n - 1)
        ...
        def f(lambda_ds):
            ...
            p_1 = (np.array([lambda_1 + (1 - lambda_1) * (0.5*self.n - 1) / (self.n - 1), (1 - lambda_1) * (0.5*self.n) / (self.n - 1)]) + self.epsilon_o/2) / (1. + self.epsilon_o)
            p_2 = (np.array([lambda_2 + (1 - lambda_2) * (0.5*self.n - 1) / (self.n - 1), (1 - lambda_2) * (0.5*self.n) / (self.n - 1)]) + self.epsilon_o/2) / (1. + self.epsilon_o)
        ...
        def gradf(lambda_ds):
            ...
            # gradient with respect to lambda_1
            gradf_lambda_1_val = gradf_lambda_1_val - alpha*R * (0.5*self.n/(self.n - 1)) * np.exp(phi_alpha_min - phi_alpha).dot(np.array([1, -1])) / \
                                                                                                    (np.exp(phi_alpha_min - phi_alpha).dot(p_1) * (1. + self.epsilon_o))

            # gradient with respect to lambda_2
            gradf_lambda_2_val = gradf_lambda_2_val - alpha*R * (0.5*self.n/(self.n - 1)) * np.exp(phi_alpha - phi_alpha_max).dot(np.array([1, -1])) / \
    ...
    def find_fidelity_estimator(self):
        ...
        p_1_opt = (np.array([lambda_1_opt + (1 - lambda_1_opt) * (0.5*self.n - 1) / (self.n - 1), (1 - lambda_1_opt) * (0.5*self.n) / (self.n - 1)]) + self.epsilon_o/2) / (1. + self.epsilon_o)
        p_2_opt = (np.array([lambda_2_opt + (1 - lambda_2_opt) * (0.5*self.n - 1) / (self.n - 1), (1 - lambda_2_opt) * (0.5*self.n) / (self.n - 1)]) + self.epsilon_o/2) / (1. + self.epsilon_o)
...
def generate_stabilizer_minimax_measurement_outcomes(rho, sigma, R, num_povm_list, epsilon_o):
    """
        Generates the outcomes (index pointing to appropriate POVM element) for the minimax optimal measurement strategy for stabilizer states.
        The strategy involves uniformly sampling the non-identity elements in the stabilizer group, measuring them,and only using the
    ...
    pauli_weight_list = [(count + 1, np.conj(rho).dot(W)) for (count, W) in\
                                                    enumerate(generate_Pauli_operator(nq, list(range(1, 4**nq)), flatten = True))]

    # find the largest 2^nq - 1 weights (corresponds to stabilizer group elements), and measure the corresponding Pauli operators
    pauli_measurements = sorted(pauli_weight_list, key = lambda x: np.abs(x[1]), reverse = True)[:(2**nq - 1)]

    # if G1, G2 are stabilizer generators, then S = G1G2 is an element of the stabilizer group
    # we can write S = phase W, where W is a pauli operator and phase = exp(i*phi) (phi \in {0, pi/2, pi, 3\pi/2})
    # since S |psi> = |psi>, where rho = |psi><psi|, tr(S rho) = 1
    # then, tr(W rho) = 1/phase
    # --- A note on phase for stabilizer group elements ---
    # while elements of the Pauli group can have a phase of +i or -i, elements in the stabilizer group have a phase of only +1 or -1
    # this is because any Pauli operator with a phase of +i or -i will only have eigenvalues +i or -i (the operators become anti-Hermitian)
    # since the stabilizer corresponds to +1 eigenvalue of every element in stabilizer group, the phase cannot be +i or -i
    pauli_phases = [1 / pauli_weight for (_, pauli_weight) in pauli_measurements]

    # replace the weight in pauli_measurements with the calculated phase
    pauli_measurements = [(pauli_index, pauli_phase) for ((pauli_index, _), pauli_phase) in zip(pauli_measurements, pauli_phases)]

    # the effective POVM for minimax optimal strategy consists of just two POVM elements
    # however, the actual measurements performed are 'R' Pauli measurements which are uniformly sampled from the stabilizer group (excluding identity)
    # np.random.choice doesn't allow list of tuples directly, so indices sampled instead
    # see https://stackoverflow.com/questions/30821071/how-to-use-numpy-random-choice-in-a-list-of-tuples/55517163
    uniformly_sampled_indices = np.random.choice(len(pauli_measurements), size = int(R))
...
def fidelity_estimation_stabilizer_state_minimax_measurement(nq = 2, generators = ['XX', 'ZZ'], num_povm_list = 2, R = 100, epsilon = 0.05, risk = None, epsilon_o = 1e-5, noise = True,\
                                                             noise_type = 'depolarizing', tol = 1e-6, random_seed = 1, verify_estimator = False, print_result = True, write_to_file = False,\
                                                             dirpath = './Data/Computational/', filename = 'temp'):
    """
        Generates the nq-qubit stabilizer state defined by the specified stabilizer generators, and finds an estimator for fidelity using Juditsky & Nemirovski's approach, specialized to
        this case. The specialized approach allows for computation of the estimator for very large dimensions.

        Minimax optimal measurement strategy for stabilizer states is implemented. This involves uniformly sampling the non-identity elements in the stabilizer group, measuring them,
        and only using the eigenvalue (either +1 or -1) of the measured outcome.
        This random sampling is accounted for by a single POVM, so number of types of measurement (N) is just one.
        The estimator and the risk only depend on the dimension, the number of repetitions, and the confidence level, and is independent of the actual stabilizer state used.
    ...
    if risk is not None:
        if risk < 0.5:
            R = int(np.ceil(2*np.log(2/epsilon) / np.abs(np.log(1 - n**2 / (n - 1)**2 * risk**2))))
        else:
    ...
    effective_outcomes = generate_stabilizer_minimax_measurement_outcomes(rho, sigma, R, num_povm_list, epsilon_o)

    ### obtain the fidelity estimator
    MSFEM = Minimax_Stabilizer_Fidelity_Estimation_Manager(n, R, epsilon, epsilon_o, tol)
    fidelity_estimator, risk = MSFEM.find_fidelity_estimator()
    ...
    if verify_estimator:
        # the effective POVM for the optimal measurement strategy is simply {rho + (n/2 - 1)/(n - 1) Delta, (n/2)/(n - 1) Delta},
        # where Delta = I - rho
        Delta = np.eye(2**nq).ravel() - rho
        POVM_list = [[rho + ((n/2 - 1)/(n - 1)) * Delta, ((n/2)/(n - 1)) * Delta]]
        ...
        constraint_general = np.real(np.sum([np.sqrt(np.conj(Ei).dot(sigma_1_opt)*np.conj(Ei).dot(sigma_2_opt)) for Ei in POVM_list[0]]))
